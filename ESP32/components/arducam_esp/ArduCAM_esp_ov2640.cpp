...
void ArduCAM_write_reg(void *p, uint8_t addr, uint8_t data){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->write_reg(addr, data);
}

uint8_t ArduCAM_read_reg(void *p, uint8_t addr){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->read_reg(addr);
}

byte ArduCAM_wrSensorReg8_8(void *p, int regID, int regDat){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->wrSensorReg8_8(regID, regDat);
}

byte ArduCAM_rdSensorReg8_8(void *p, uint8_t regID, uint8_t *regDat){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->rdSensorReg8_8(regID, regDat);
}

void ArduCAM_flush_fifo(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->flush_fifo();
}
void ArduCAM_clear_fifo_flag(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->clear_fifo_flag();
}

void ArduCAM_start_capture(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->start_capture();
}

void ArduCAM_set_bit(void *p, uint8_t addr, uint8_t bit){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->set_bit(addr, bit);
}

uint8_t ArduCAM_get_bit(void *p, uint8_t addr, uint8_t bit){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->get_bit(addr, bit);
}

uint32_t ArduCAM_read_fifo_length(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->read_fifo_length();
}

void ArduCAM_CS_LOW(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->CS_LOW();
}

void ArduCAM_CS_HIGH(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	return cam->CS_HIGH();
}

void ArduCAM_OV2640_InitCAM(void *p){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->InitCAM();
}

void ArduCAM_set_format(void *p, byte fmt){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
	cam->set_format(fmt);
}

void ArduCAM_OV2640_set_JPEG_size(void *p, int size = OV2640_320x240){
	ArduCAM_OV2640 *cam = (ArduCAM_OV2640 *)p;
