...
esp_err_t spi_master_init(void)
{
    ...
    spi_device_interface_config_t devcfg = {
        .command_bits = 8,
        .queue_size = 1,
        .clock_speed_hz = 6000000, //must be strictly slow than ArduChip SPI 8MHz
        .mode = 0,
...
size_t spi_transfer_bytes(uint8_t command, uint8_t *send_buf, uint8_t *receive_buf, size_t size)
{
    spi_transaction_t t = {
        .cmd = command,
        .length = size * 8,
